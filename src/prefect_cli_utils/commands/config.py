...
from prefect_cli_utils import config as pipeline_config
from prefect_cli_utils.config import load_config
from prefect_cli_utils.const import _ENV_VAR_MAPPING

logger = logging.getLogger("prefect_cli_utils.commands.config")

config = typer.Typer(
    help="🗒  Create and/or validate a config file for a prefect_cli_utils pipeline.",
...
@config.command(
    name="create",
    short_help="🗒  Create a configuration file to run a prefect_cli_utils pipeline.",
)
def create_config(
...
@config.command(
    name="validate",
    short_help="✅ Validate a configuration file for a prefect_cli_utils pipeline.",
)
def validate_config(
